...
static void save_last_oid(struct sdata *sd) {
    ...
    mdb_put(sd->txn, sd->jdbi, &key, &data, 0);
}
...
static void load_sdata_info(struct sdata *sd) {
    ...
    key.mv_size = sizeof(x);
    key.mv_data = &x;
    if (mdb_get(sd->txn, sd->jdbi, &key, &data) == 0) {
...
static char *compress(const char *src, int *compressed_len) {
...
static void sdata_add_object(struct sdata *sd, json_t *j) {
    ...
    int compressed_len;
    char *compressed_data = compress(jdata, &compressed_len);
    MDB_val key, data;
...
struct sdata *sdata_new(struct shard *shard) {
    ...
    if (mdb_dbi_open(s->txn, "json", MDB_CREATE|MDB_INTEGERKEY, &s->jdbi) != 0) {
        M_ERR("Failed to load dbi");
    } else {
        load_sdata_info(s);
    }
